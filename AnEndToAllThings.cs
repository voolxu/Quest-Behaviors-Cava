...
namespace Honorbuddy.Quest_Behaviors.Cava.AnEndToAllThings
{
	/// <summary>
	...
	/// </summary>
	[CustomBehaviorFileName(@"Cava\AnEndToAllThings")]
	public class AnEndToAllThings : QuestBehaviorBase
	{
			...
			: base(args)
		{
			try
			{
				...
				ActionBarIndexAttack = GetAttributeAsNullable("AttackSpellIndex", true, ConstrainAs.SpellId, null) ?? 0;
				ActionBarIndexHeal = GetAttributeAsNullable("HealSpellIndex", false, ConstrainAs.SpellId, null) ?? 0;
				HealNpcId = GetAttributeAsNullable("HealNpcId", true, ConstrainAs.MobId, null) ?? 0;
				ItemIdToSummonVehicle = GetAttributeAsNullable("ItemId", false, ConstrainAs.ItemId, null) ?? 0;
				KillNpcId = GetAttributeAsNullable("KillNpcId", true, ConstrainAs.MobId, null) ?? 0;
				VehicleId = GetAttributeAsNullable("VehicleId", true, ConstrainAs.VehicleId, null) ?? 0;

				AuraIdRideVehicleHardcoded = 46598;    // http://wowhead.com/spell=46598
				FlightorMinHeight = 25.0;
				MobIdFrostbroodVanquisher = 28670;     // http://wowhead.com/npc=28670
				MobIdScarletBallista = 29104;          // http://wowhead.com/npc=29104
				MobIdTirisfalCrusader = 29103;         // http://wowhead.com/npc=29102
		...
		private int ActionBarIndexAttack { get; set; }
		private int HealNpcId { get; set; }
		private int ActionBarIndexHeal { get; set; }
		...
		private int AuraIdRideVehicleHardcoded { get; set; }
		private WoWPoint? StationPoint { get; set; }
		private double FlightorMinHeight { get; set; }
		private WoWItem ItemToSummonVehicle { get; set; }
		private int MobIdFrostbroodVanquisher { get; set; }
		private int MobIdScarletBallista { get; set; }
		private int MobIdTirisfalCrusader { get; set; }
		...
		private VehicleAbility WeaponDevourHuman { get; set; }
		private VehicleWeapon WeaponFrozenDeathbolt { get; set; }
		...
		public override string SubversionId { get { return ("$Id: AnEndToAllThings.cs 1362 2014-04-03 18:25:33Z chinajade $"); } }
		public override string SubversionRevision { get { return ("$Revision: 1362 $"); } }
			...
			return new PrioritySelector(
				...
				new Decorator(context => !Me.IsCasting,
					new ActionFail(context =>
					{
						...
						WeaponDevourHuman = null;
						WeaponFrozenDeathbolt = null;
		...
		private Composite StateBehaviorPS_RidingOutToHuntingGrounds()
		{
			return new PrioritySelector(
				...
				new Decorator(context => WeaponFrozenDeathbolt.IsWeaponUsable(),
				...
				new Action(context =>
				{
					...
					if (StationPoint.HasValue)
					{
						if (myLocation.Distance(StationPoint.Value) > FlyingPathPrecision)
						{
                            ...
                            Navigator.PlayerMover.MoveTowards(StationPoint.Value);
                            return RunStatus.Success;
						}
					...
					if (!StationPoint.HasValue)
					{
						...
						if (!WeaponFrozenDeathbolt.IsWeaponReady()
					...
					if (distance2DSqrToTarget > (TargetDistance2DMax * TargetDistance2DMax))
					{
                        ...
                        Navigator.PlayerMover.MoveTowards(FindDistanceClosePoint(SelectedTarget, PathPatrol.Peek().Z));
                        return RunStatus.Success;
					}
					...
					if (distance2DSqrToTarget < (TargetDistance2DMin * TargetDistance2DMin))
					{
                        ...
                        Navigator.PlayerMover.MoveTowards(FindDistanceGainPoint(SelectedTarget, TargetDistance2DMin));
                        return RunStatus.Success;
					}
		...
		private Composite StateBehaviorPS_ReturningToBase()
		{
			return new PrioritySelector(
				...
				new Action(context =>
				{
					...
					Navigator.PlayerMover.MoveTowards(PathEnd);
				})
			);
		}
				...
				new Decorator(context =>
					{
						...
						return
							WeaponDevourHuman.IsAbilityReady()
					...
					new Action(context =>
					{
						...
						if (selectedTargetLocation.Distance(DragonVehicle.Location) > (WeaponDevourHuman.MaxRange / 2))
						{
							//Flightor.MoveTo(selectedTargetLocation);
                            Navigator.PlayerMover.MoveTowards(selectedTargetLocation);
                            return RunStatus.Success;
						}

						WoWMovement.MoveStop();
						WeaponDevourHuman.UseAbility();
		...
		{
			return
				new Decorator(context => (WeaponDevourHuman == null)
											|| (WeaponFrozenDeathbolt == null),
				...
				// then it will cause the WoWclient to d/c.
					new WaitContinue(TimeSpan.FromSeconds(10),
						...
						new Action(context =>
						{
							...
							WeaponFrozenDeathbolt =
								new VehicleWeapon(ActionBarIndexAttack, weaponArticulation, WeaponMuzzleVelocity)
								{
									LogAbilityUse = true,
							...
							WeaponDevourHuman =
								new VehicleAbility(ActionBarIndexHeal)
								{
									LogAbilityUse = true
								};
		...
		private WoWPoint FindNewStationPoint(WoWUnit desiredTarget, double offsetRadians = 0.0)
		{
			...
			{ desiredTarget = FindMobToKill(MobIdScarletBallista); }
			...
			var escapePoint = targetLocation.RayCast(escapeHeading, preferredDistance);
		...
		private bool IsViableTarget(WoWUnit wowUnit)
		{
			...
			return
				...
				&& wowUnit.IsUntagged()
				&& !wowUnit.HasAura(AuraIdRideVehicleHardcoded)
