...
namespace Honorbuddy.Quest_Behaviors.SpecificQuests.Cava.ThePitOfScales
{
    [CustomBehaviorFileName(@"Cava\27738-Uldum-ThePitOfScales")]
    public class ThePitOfScales : CustomForcedBehavior
    {
        ...
        private static List<WoWUnit> Ready_Tahet { get { return ObjectManager.GetObjectsOfType<WoWUnit>().Where(ret => (ret.Entry == 46496 && !ret.HasAura(101422))).OrderBy(ret => ret.Distance).ToList(); } }
        private static List<WoWUnit> Boss_Caimas { get { return ObjectManager.GetObjectsOfType<WoWUnit>().Where(ret => (ret.Entry == 46276 && ret.IsAlive && ret.HasAura(86595))).OrderBy(ret => ret.Distance).ToList(); } }
        ...
        public override string SubversionId { get { return "$Id: 27738-Uldum-ThePitOfScales.cs 1188 2014-01-02 23:33:57Z mainhaxor $"; } }
        public override string SubversionRevision { get { return "$Rev: 1188 $"; } }
        #endregion
                    ...
                    new Decorator(context => (!Me.Combat && Ready_Tahet == null) || (!Me.Combat && FindCompetingPlayers().Count() > 0 && Ready_Tahet.Count > 0),
                        new PrioritySelector(
                            ...
                            new Decorator(context => Boss_Caimas.Count > 0 && Boss_Caimas[0].Location.Distance(Me.Location) <= 25,
                                new Action(context =>
                                {
                                    QBCLog.Info("Atacking Boss to get credit and complet quest");
                                    var spell = "";
                                    switch (Me.Class)
                                    {
                                        case WoWClass.Mage:
