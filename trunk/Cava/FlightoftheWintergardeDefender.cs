...
namespace Honorbuddy.Quest_Behaviors.Cava.FlightoftheWintergardeDefender
{
    [CustomBehaviorFileName(@"Cava\FlightoftheWintergardeDefender")]
    public class FlyingVehicle : CustomForcedBehavior
    {
        ...
        public override string SubversionId { get { return ("$Id: FlyingVehicle.cs 1527 2014-05-29 12:01:53Z Dogan $"); } }
        public override string SubversionRevision { get { return ("$Revision: 1527 $"); } }
        ...
        public WoWUnit HavePassanger { get { return ObjectManager.GetObjectsOfType<WoWUnit>().FirstOrDefault(ret => NpcList.Contains((int)ret.Entry) && ret.Distance < 2 && ret.IsOnTransport); } }
                        ...
                        while (true)
                        {
                            ...
                            switch (_state)
                            {
                                ...
                                case State.landing:
                                    ...
                                    {
                                        while (HavePassanger != null)
                                        {
                                            WoWMovement.MoveStop();
                                            StyxWoW.SleepForLagDuration();
                                            Lua.DoString(
                                                "if GetPetActionCooldown({0}) == 0 then CastPetAction({0}) end",
                                                DropPassengerButton);
                                            StyxWoW.SleepForLagDuration();
                                            StyxWoW.Sleep(2000);
                                            _state = State.start;
