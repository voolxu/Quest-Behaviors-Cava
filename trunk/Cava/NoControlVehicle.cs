...
namespace Honorbuddy.Quest_Behaviors.Cava.NoControlVehicle
{
	[CustomBehaviorFileName(@"Cava\NoControlVehicle")]
	...
	public class NoControlVehicle : CustomForcedBehavior
	{
			...
			: base(args)
		{
			...
			try
			{
				...
				AttackButton = GetAttributeAsNullable("AttackButton", true, ConstrainAs.HotbarButton, new[] { "AttackIndex", "SpellIndex" }) ?? 0;
				AttackButton2 = GetAttributeAsNullable("AttackButtonSecondary", false, ConstrainAs.HotbarButton, new[] { "AttackIndexSecondary", "SpellIndexSecondary" }) ?? 0;
				GoHomeButton = GetAttributeAsNullable("GoHomeButton", false, ConstrainAs.HotbarButton, new[] { "HomeIndex" }) ?? 0;
				MaxRange = GetAttributeAsNullable("MaxRange", false, ConstrainAs.Range, null) ?? 1;
				MountedPoint = WoWPoint.Empty;
				NumOfTimes = GetAttributeAsNullable("NumOfTimes", false, ConstrainAs.RepeatCount, new[] { "TimesToUse" }) ?? 1;
				OftenToUse = GetAttributeAsNullable("OftenToUse", false, ConstrainAs.Milliseconds, null) ?? 1000;
				QuestId = GetAttributeAsNullable("QuestId", false, ConstrainAs.QuestId(this), null) ?? 0;
				SpellType = GetAttributeAsNullable("TypeId", false, new ConstrainTo.Domain<int>(0, 5), null) ?? 2;
				TargetIds = GetNumberedAttributesAsArray("TargetId", 1, ConstrainAs.MobId, new[] { "MobId", "NpcId" });
				TargetIdsSecondary = GetNumberedAttributesAsArray("TargetIdSecondary", 0, ConstrainAs.MobId, new[] { "MobIdSecondary", "NpcIdSecondary" });
				VehicleId = GetAttributeAsNullable("VehicleId", false, ConstrainAs.VehicleId, null) ?? 0;
				VehicleMountId = GetAttributeAsNullable("VehicleMountId", false, ConstrainAs.VehicleId, new[] { "NpcMountId", "NpcMountID" }) ?? 1;
				WaitTime = GetAttributeAsNullable("WaitTime", false, ConstrainAs.Milliseconds, null) ?? 0;
	    ...
	    private int AttackButton { get; set; }
	    private int AttackButton2 { get; set; }
	    private int GoHomeButton { get; set; }
	    private double MaxRange { get; set; }
        // ReSharper disable once UnusedAutoPropertyAccessor.Local
	    private WoWPoint MountedPoint { get; set; }
	    private int OftenToUse { get; set; }
	    ...
	    private int SpellType { get; set; }
	    private int[] TargetIds { get; set; }
	    private int[] TargetIdsSecondary { get; set; }
	    private int NumOfTimes { get; set; }
	    private int WaitTime { get; set; }
	    private int VehicleId { get; set; }
	    private int VehicleMountId { get; set; }
		...
		private IEnumerable<WoWUnit> NpcList
		{
			get
			{
		...
		private IEnumerable<WoWUnit> NpcListSecondary
		{
			get
			{
				if (VehicleList.Count > 0)
		...
		protected override Composite CreateBehavior()
		{
			return _root ??
				(_root = new PrioritySelector(
                    new Decorator(c => (Counter > NumOfTimes) || Me.IsQuestComplete(QuestId) || (_doingQuestTimer.ElapsedMilliseconds >= 180000),
						new Action(c =>
						{
        ...
        private async Task<bool> TypeTwoVehicleBehavior()
        {
            ...
            while (Me.IsAlive)
            {
                if (Counter > NumOfTimes && QuestId == 0 || Me.IsQuestComplete(QuestId) || (_doingQuestTimer.ElapsedMilliseconds >= 180000))
                {
                    Lua.DoString("VehicleExit()");
                    _isBehaviorDone = true;
                    return true;
